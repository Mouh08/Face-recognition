...
f2 = zeros(3,3,8);
f2(:,:,1) = [-3 -3 5;-3 0 5;-3 -3 5];
f2(:,:,2) = [-3 5 5;-3 0 5;-3 -3 -3];
f2(:,:,3) = [5 5 5;-3 0 -3;-3 -3 -3];
f2(:,:,4) = [5 5 -3;5 0 -3;-3 -3 -3];
f2(:,:,5) = [5 -3 -3;5 0 -3;5 -3 -3];
f2(:,:,6) = [-3 -3 -3;5 0 -3;5 5 -3];
f2(:,:,7) = [-3 -3 -3;-3 0 -3;5 5 5];
f2(:,:,8) = [-3 -3 -3;-3 0 5;-3 5 5];
...
for j = 1:8
    for i = 1:8
        clip = im2(round((j-1)*ys/8+1):round(j*ys/8),round((i-1)*xs/8+1):round(i*xs/8));
        eoh(j,i,:) = hist(clip(:), 0:8);%permute(hist(clip(:), 0:8), [1 3 2]);
    end
end
...
for i=1:8
    for j=1:8
        d1=[d1    eoh(i,j,1)  eoh(i,j,2) eoh(i,j,3) eoh(i,j,4) eoh(i,j,5) eoh(i,j,6) eoh(i,j,7) eoh(i,j,8)];
    end
end
